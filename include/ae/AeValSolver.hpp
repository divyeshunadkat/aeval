...
namespace ufo
{
  ...
  public:
    ...
    AeValSolver (Expr _s, Expr _t, ExprSet &_v, bool _debug = false, bool _skol = true, bool _opt_max = false) :
      s(_s), t(_t), v(_v),
      efac(s->getFactory()),
      z3(efac),
      smt (z3),
     ...
     */
    Expr getValidSubset(bool compact = false)
    {
    ...
    {//outs () << "getSkolemFunction: " << *var << "  " << *exp << "\n";
      if (!isNumeric(var))
      {
        if (isOpX<EQ>(exp))
        {
...
//      if (debug) outs () << "getAssignmentForVar " << *var << " in:\n" << *exp << "\n";
    ...
    Expr compositeAssm(ExprSet& cnjs, Expr var, bool isInt)
    {
        ...
        if (conjNEQ.size() == 0)
        {
          if (isInt)
          {
            if (curMax != NULL && curMin != NULL) return (opt_max ? curMin : curMax);
