...
(rule (inv0 a 0 20 10 n elem))

(rule (=> (and (inv0 a i C1 C2 n elem) (< i n)
  (= a0 (ite (= i 0) (store a i C1) (store a i (+ (select a (- i 1)) C2))))
  (= i1 (+ i 1))) (inv0 a0 i1 C1 C2 n (select a0 i))))

(rule (=> (and (inv0 a0 i C1 C2 n elem) (>= i n)) (inv1 a 0 C1 C2 n elem)))

(rule (=> (and (inv1 a i C1 C2 n elem) (< i (div n 2))
  (= a1 (store a i (+ (* i C2) C1)))
  (= i1 (+ i 1))) (inv1 a1 i1 C1 C2 n (select a1 i))))

(rule (=> (and (inv1 a i C1 C2 n elem) (>= i (div n 2))) (inv2 a i C1 C2 n elem))) 

(rule (=> (and (inv2 a i C1 C2 n elem) (< i n)
  (= a2 (store a i (+ (* i C2) C1)))
  (= i1 (+ i 1))) (inv2 a2 i1 C1 C2 n (select a2 i))))

(rule (=> (and (inv2 a2 i C1 C2 n elem) (inv a0 i C1 C2 n elem) (>= i n)
